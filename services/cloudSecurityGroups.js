...
function logic(data) {
  ...
  obj.groups = [];
  obj.loading = false;
  var AtomicRule = function (account, account_id, regionId, vpc, sgName, sgId, sgExternalId, protocol, port, scope, direction, tags, instancesCount, elbsCount) {
      ...
      _.each(instance.nics, function (nic) {
        _.each(nic.securityGroupIds, function (sgID) {
          if (obj.instancesSGMap[sgID]) {
            obj.instancesSGMap[sgID].push({"instanceName": instance.name, "InstanceID": instance.instanceId});
          }
          else {
      ...
      var inboundRules = _.map(sg.services.inbound, function (service) {
        return _.map(service.scope, function (scopeItem) {
          ...
          var ruleToAdd = new AtomicRule(
            ...
            service.port,
            scopeItem,
            "Inbound", sg.tags, instancesCount, elbsCount);
      ...
      var outboundRules = _.map(sg.services.outbound, function (service) {
        return _.map(service.scope, function (scopeItem) {
          ...
          var ruleToAdd = new AtomicRule(
            ...
            service.port,
            scopeItem,
            "Outbound", sg.tags, instancesCount, elbsCount);
  ...
  obj.fieldsToAutoCompleate = ["elbsCount", "instancesCount", "account_id", "account", "region", "vpc", "sgName", "sgId", "xId", "protocol", "port", "direction", "type", "source"];
